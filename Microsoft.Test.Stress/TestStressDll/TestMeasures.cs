...
21968064,
        ...
        [TestMethod]
        public async Task TestSampleDataOutlier()
        {
            if (StressUtilOptions.IsRunningOnBuildMachine())
            {
                return;
...
15270968,
                ...
                SampleType.SampleTypeIteration))
            {
                ...
                LogMessage($"# Iter = {measurementHolder.measurements[PerfCounterType.GCBytesInAllHeaps].Count}");
                var leakAnalysisResults = await measurementHolder.CalculateLeaksAsync(showGraph: true);
                LogMessage($" Num outliers = {leakAnalysisResults[0].NumOutliers}");
                foreach (var x in leakAnalysisResults[0].lstData)
                {
                    LogMessage($" {x.IsOutlier,5} {x.point} {x.distance}");
                }
                await measurementHolder.CalculateMinimumNumberOfIterationsAsync(leakAnalysisResults);
            }
