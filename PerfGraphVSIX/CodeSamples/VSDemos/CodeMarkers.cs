//Desc: Shows how to fire codemarkers
    ...
    public class MyClass : MyCodeBaseClass
    {
        ...
        async Task DoItAsync()
        {
            ...
            while (!_CancellationTokenExecuteCode.IsCancellationRequested)
            {
                ...
                //                        CreateDynamicTracingCodeMarkerData(trace, CodeMarkerAction.None));
            ...
            /*
            Name
            + Process64 devenv (15184) Args:  /rootsuffix Exp
             + Thread (16648) CPU=57ms
              + ntdll!?
               + kernel32!?
                + devenv!?
                 + msenv!?
                   ...
                   + ?!?
                    + windowsbase.ni!?
                     + mscorlib.ni!?
                      + windowsbase.ni!?
                       + mscorlib.ni!?
                        + tmpcea6!Microsoft.Internal.Performance.MyClass+<DoIt>d__3.MoveNext()
                         + tmpcea6!CodeMarkers.CodeMarker
                          + tmpcea6!dynamicClass.IL_STUB_PInvoke(int,unsigned int8[],int)
                           + microsoft.visualstudio.codemarkers!?
                            + ntdll!?
                             + wow64!?
                              + wow64cpu!?
                               + wow64!?
                                + ntdll!?
                                 + Event Provider(641d7f6c-481c-42e8-ab7e-d18dc5e5cb9e)/EventID(123)
             */
