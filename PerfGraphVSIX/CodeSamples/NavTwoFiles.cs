...
namespace MyCodeToExecute
{
    public class MyClass : BaseExecCodeClass
    {
        ...
        public override async Task DoInitializeAsync()
        {
            ...
            await Task.Delay(500, _CancellationTokenExecuteCode); // wait one second to allow UI thread to catch  up
        ...
        public override async Task DoIterationBodyAsync()
        {
            Action<string> DoScrolling = async (file) =>
            {
                try
                {
                    ...
                    for (int r = 0; r < nScroll && !_CancellationTokenExecuteCode.IsCancellationRequested; r++)
                    {
                        ...
                        await Task.Delay(1000, _CancellationTokenExecuteCode); // wait one second to allow UI thread to catch  up
                    }
            ...
            DoScrolling(file1);
            await Task.Delay(2000, _CancellationTokenExecuteCode);
            DoScrolling(file2);
        }
